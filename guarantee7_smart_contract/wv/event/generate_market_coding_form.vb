...
Public Class generate_market_coding_form

    Shared directory_market As String = Regex.Replace(Directory.GetCurrentDirectory, "guarantee7\\guarantee7.+", "guarantee7\guarantee7_smart_contract\wv\market")

    Public Shared Sub exe()

        If Not GRT.GR.account.login_state = "no_login" Then

            GR.control.wv_working.CoreWebView2.Navigate(directory_market & "\html\coding.html")
    ...
    Shared Async Sub UpdateContent(ByVal sender As Object, ByVal args As CoreWebView2WebMessageReceivedEventArgs)
        ...
        Select Case json_webview("key").ToString
            ...
            Case "initialize_textarea_code_market"

                ' html

                Dim file_info As New FileInfo(directory_market & "\html\wallet_source.html")
                ...
                Dim directory_info As New DirectoryInfo(directory_market & "\js\wallet")
                ...
                directory_info = New DirectoryInfo(directory_market & "\css\wallet")
                ...
                file_info = New FileInfo(directory_market & "\etc\mysql_query.txt")
